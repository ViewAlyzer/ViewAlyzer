/**
 * @file tracing_user.h
 * @brief ViewAlyzer replacement for Zephyr's stock tracing_user.h
 *
 * This header is a drop-in replacement for the stock Zephyr
 * subsys/tracing/user/tracing_user.h.  It adds macro wiring for
 * mutex, semaphore, and message-queue tracing that the stock header
 * leaves as empty stubs.
 *
 * Usage:
 *   In your CMakeLists.txt, BEFORE find_package(Zephyr):
 *     (nothing special needed)
 *   AFTER find_package(Zephyr):
 *     zephyr_include_directories(BEFORE <path-to-this-directory>)
 *
 *   Because the include uses BEFORE, this header will be found
 *   before the stock one.  The identical include-guard
 *   (_TRACE_USER_H) prevents the stock header from being processed
 *   a second time.
 *
...
/* ── Thread / ISR / idle callbacks (stock Zephyr) ────────────── */
...
/* ── Thread / ISR / idle dispatch (stock Zephyr) ─────────────── */
...
/* ────────────────────────────────────────────────────────────────
 *  ViewAlyzer additions: mutex / semaphore / message-queue
 *  dispatch-function declarations.
 *
 *  These are defined in VA_Adapter_Zephyr.c (not in stock
